	...
	public static void main (String[] args)  throws Exception
	{
		...
		{ 
			...
			try
			{
                ...
                statement.execute("DROP TABLE IF EXISTS inventory, ParkingLot, ParkingLotType, ParkingSpot, ParkingSpotType, Vehicle, VehicleType, Payment;");
                System.out.println("Finished dropping tables (if existed).");
				
				statement.execute("CREATE TABLE ParkingLotType (ID INT, Type VARCHAR(20) NOT NULL, PRIMARY KEY(ID));");
				System.out.println("Created ParkingLotType Table");

				statement.execute("CREATE TABLE ParkingSpotType (ID INT, Type VARCHAR(10) NOT NULL, PRIMARY KEY(ID));");
				System.out.println("Created ParkingSpotType Table");

				statement.execute("CREATE TABLE VehicleType (ID INT, Type VARCHAR(30), PRIMARY KEY(ID));");
				...
				statement.execute("CREATE TABLE Payment (ID INT, LicensePlate VARCHAR(8), Duration INT NOT NULL, StartTime DATETIME NOT NULL, PRIMARY KEY(ID), FOREIGN KEY(LicensePlate) REFERENCES Vehicle(LicensePlate));");
				System.out.println("Created Payment Table");

                statement.execute("CREATE TABLE ParkingLot(ID INT, ParkingLotTypeID INT NOT NULL, Name VARCHAR(20) NOT NULL, PRIMARY KEY(ID), FOREIGN KEY(ParkingLotTypeID) REFERENCES ParkingLotType(ID));");
				System.out.println("Created ParkingLot Table");

				statement.execute("CREATE TABLE ParkingSpot (ID INT, ParkingSpotTypeID INT NOT NULL, ParkingLotID INT NOT NULL, LicensePlate VARCHAR(8), SpotNumber INT NOT NULL, Level INT NOT NULL, PRIMARY KEY(ID), FOREIGN KEY(ParkingSpotTypeID) REFERENCES ParkingSpotType(ID), FOREIGN KEY(ParkingLotID) REFERENCES ParkingLot(ID), FOREIGN KEY(LicensePlate) REFERENCES Vehicle(LicensePlate));");
